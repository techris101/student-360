# Student 360 — AI Prompts and Rules

All prompts live in `src/lib/ai/prompts.ts`, copied verbatim from this file. Variables in `{{double_braces}}`. All structured outputs are validated with zod; invalid output gets one retry, then fails loudly in logs.

## 1. Advisor system prompt (model: AI_MODEL_SMART)
```
You are the Student 360 Advisor: a career and study advisor for university students in Rwanda.
...
```
Context assembly: profile compacted to under 600 tokens; last 20 messages of the thread; matched list from the ranking engine. Output limit 700 tokens.

## 2. Opportunity extraction (model: AI_MODEL_FAST)
```
You extract structured data about opportunities for university students in Rwanda from a web page.
...
{{page_text}}
```
Schema (zod): `is_opportunity, type, title, organisation, summary, key_facts[], official_url, deadline, deadline_rolling, opens_at, starts_at, location_scope, location_text, funding, levels[], fields[], eligibility{...as in ARCHITECTURE.md}, plan_ahead, prepare_now[], confidence`.

## 3. News extraction (model: AI_MODEL_FAST)
```
You decide whether a news article matters to university students in Rwanda and summarise it.
...
## 4. CV parsing (model: AI_MODEL_FAST)
```
Extract profile fields from this CV text for a Rwandan university student.
...
{{cv_text}}
```
The student reviews every proposed field before it is saved. Raw CV text is not stored; `cv_parsed` stores the confirmed JSON.

## 5. Progress review (model: AI_MODEL_SMART)
```
Write a short, honest review of this student's opportunity activity and what to do next.
...
## Cost controls
- Per-user daily cap (`AI_DAILY_MESSAGE_CAP`), enforced before calling the model.
- Pipeline cap of 200 extraction calls per run.
- Page text truncated to 12,000 characters before extraction.
- Token usage logged per call; admin sees daily totals.

## Privacy note
The Gemini API free tier may use submitted content to improve Google products. Before real students' CVs flow through it, the owner must enable billing (paid tier). Log this as a launch blocker in `PROGRESS.md` if billing is not confirmed.
